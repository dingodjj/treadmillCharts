...
import com.nl.treadmilllauncher.utils.ColorUtil;
...
public class StringScrollPicker extends ScrollPickerView<CharSequence> {
    ...
    private int mMidColor = Color.GRAY; // 上下两边的颜色
    ...
    public StringScrollPicker(Context context, AttributeSet attrs,
        ...
        mPaint.setTypeface(CommonUtils.getTfByPath(context, "fonts/MyriadPro-BoldCond.otf"));
        init(attrs);
    ...
    private void init(AttributeSet attrs) {
        if (attrs != null) {
            ...
            mMidColor = typedArray.getColor(
                    R.styleable.StringScrollPicker_spv_mid_color, mMidColor);
    ...
    public void drawItem(Canvas canvas, List<CharSequence> data, int position, int relative, float moveLength, float top) {
        ...
        StaticLayout layout = new StaticLayout(text, 0, text.length(), mPaint,
                mMaxLineWidth, mAlignment, 1.0F, 0.0F, true, null, 0);
        float x = 0;
        float y = 0;
        ...
        if (relative == -1 || relative == 1) { // 上一个或下一个
            ...
            if ((relative == -1 && moveLength < 0)
                ...
                float rate = (itemSize - Math.abs(moveLength))
                        / itemSize;
                color = ColorUtil.computeGradientColor(mStartColor, mMidColor, rate);
            }
        }if (relative == -2 || relative == 2) { // 上一个或下一个
            // 处理上一个item且向上滑动　或者　处理下一个item且向下滑动　，颜色为mEndColor
            if ((relative == -2 && moveLength < 0)
                    || (relative == 2 && moveLength > 0)) {
                color = mEndColor;
            } else { // 计算渐变的颜色
                float rate = (itemSize - Math.abs(moveLength))
                        / itemSize;
                color = ColorUtil.computeGradientColor(mMidColor, mEndColor, rate);
            }
